# 强化学习阶段 PPO 改进方案：LuckyJ/ACH 技术融合与局部后悔度设计

> 写于 2026-04-01，用于在进入 强化学习阶段（PPO 在线自对弈）前规划所有可引入的改进。  
> 本文档是自包含的，不依赖对话上下文。几个月后无需任何背景即可依据本文档执行修改。
>
> 注：本文对应当前“强化学习阶段”的规划草案。文中如果出现更早阶段材料的引用，统一按当前项目命名理解为“监督学习阶段 / 强化学习阶段”。

---

## 一、本文档的定位

当前项目的强化学习入口是 `mortal/train_online.py`。本文档针对 PPO 自对弈阶段的已知可改进点进行分析和方案设计，包括：
...
以下是 强化学习阶段 入口 `mortal/train_online.py` 的关键结构（截至 2026-04-01）。
...
### P0 — 零成本稳定性改进（监督学习阶段或强化学习阶段均可立即实施）
...
**风险评估：** 低。  
**时机：** 强化学习阶段 跑出第一批结果后，如果发现 ratio 方差大再加。
...
**方式 B：作为 PPO advantage 的辅助信号（强化学习阶段）**
...
**推荐路径：** 先做方式 A（在监督学习阶段中验证辅助头的预测质量），确认预测准确后再在强化学习阶段中做方式 B。注意：方式 B 中不同后悔度维度的 penalty weight 需要单独调参，建议先只用维度 1（牌效率）做 POC。
...
| 文件 | 用途 |
|------|------|
| `mortal/train_online.py` | 强化学习阶段 PPO 训练入口，所有 P0/P1 改进在此实施 |
