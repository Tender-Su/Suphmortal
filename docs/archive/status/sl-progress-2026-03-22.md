...
当前项目并不是在推进新的主干模型结构，而是在把 `监督学习阶段` 的选模、验证和 `P1` 辅助头搜索做成更可靠的实验系统。实际进度可以概括为：
...
- 因此 `P2` 和 `监督学习阶段 formal` 还不能视为在新口径下已经定型
...
新增的 `revalidate_sl_round.py` 和相关缓存版本逻辑，说明当前一个真实工程问题已经被确认：
...
这代表 `监督学习阶段` 已经从“先跑实验再看”进入“必须保证历史结果可重放、可复核”的阶段。

## 当前已经落地并可视为证据的实验进展

### 1. `P0` 已完成新口径下的重验和 top4 收敛

当前运行目录 `logs/sl_fidelity/sl_fidelity_main/state.json` 最可靠的已固化结论是：
...
- `logs/sl_fidelity/sl_fidelity_main/p1_calibration.json`
- `logs/sl_fidelity/sl_fidelity_main/p1_calibration__s20261716.json`
...
- `logs/sl_ab/sl_fidelity_main_p1_solo_s20261817/revalidated_best_loss_final_round.json`
...
但是 `logs/sl_fidelity/sl_fidelity_main/state.json` 在最近一次落盘时只固化到了 `P0 top4`，还没有把这套新 `P1` 逻辑的完整 round 结果写进去。
...
1. `监督学习阶段 / P0` 已按新 selector 收敛出新的 `top4` 协议种子。
2. `监督学习阶段 / P1` 的比较口径、搜索空间和 survivor 规则已经完成代码升级。
3. 旧 `P1 solo` 结果已经足够证明“旧预算区间过重”，所以主线不应继续沿用旧扫法。
4. 新的 `P1 solo -> pairwise -> joint refine` 还没有以完整结果形式固定下来。
5. 因此 `P2` 去重、默认 checkpoint 选择和 `监督学习阶段 formal` 启动条件，目前都还不能视为在新口径下完成。
...
5. 最后再进入 `P2` 去重和 `监督学习阶段 formal`

在这之前，不应把旧 `solo` winner、旧 `P1` 排序或旧 `P2` 结论当成当前主线结论。

## 这次检查额外确认的事项

- `mortal.test_sl_selection`：通过
- `mortal.test_train_supervised`：通过
- `mortal.test_sl_fidelity`：通过
