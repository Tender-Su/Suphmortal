...
def resolve_enable_metadata(cfg):
    if (env_override := os.environ.get('MORTAL_1V3_ENABLE_METADATA')):
        return _coerce_bool(env_override, field_name='MORTAL_1V3_ENABLE_METADATA')
    if 'enable_metadata' in cfg:
        return bool(cfg['enable_metadata'])
    return bool(cfg.get('log_dir'))


def load_eval_engines(cfg, *, enable_metadata=None):
...
def run_main(args):
    ...
    print(f'1v3 enable_metadata={resolve_enable_metadata(cfg)}')
    if log_dir is None:
