...
def stage05_publish_targets(
    ...
    *,
    ...
    return [
        destination
        for destination, _ in stage05_publish_plan(base_cfg, payload, config_dir=config_dir)
    ]
...
def finalize_formal_result(
    ...
    *,
    ...
    if offline_winner not in result['candidates']:
        raise RuntimeError(
            f'formal result is missing offline checkpoint winner in checkpoint pack: {offline_winner!r}'
        )
    ...
    result['pending_canonical_alias_targets'] = [
        str(destination)
        for destination in stage05_publish_targets(
            config_snapshot['base_cfg'],
            {
                'winner': offline_winner,
...
def load_stage05_state(path: Path) -> dict | None:
    try:
...
def iter_stage05_state_paths() -> list[Path]:
    if not STAGE05_FIDELITY_ROOT.exists():
        return []
    state_paths = [
        path
        for path in STAGE05_FIDELITY_ROOT.glob('*/state.json')
        if path.is_file()
    ]
...
def resolve_pending_canonical_alias_targets(state: dict) -> list[str]:
    ...
    if not isinstance(candidates, dict):
        return []
    winner = str(result.get('offline_checkpoint_winner') or result.get('winner') or '').strip()
    if winner not in candidates:
        return []
...
def stage05_canonical_handoff_complete(state: dict) -> bool:
...
def find_pending_stage05_handoff_for_init_state(init_state_file: str) -> dict | None:
    resolved_init_state = Path(init_state_file).resolve()
    for state_path in iter_stage05_state_paths():
        state = load_stage05_state(state_path)
        if state is None:
            continue
        pending_targets = resolve_pending_canonical_alias_targets(state)
        if not pending_targets:
        ...
        if not isinstance(final_conclusion, dict):
            continue
        if stage05_canonical_handoff_complete(state):
            return None
        return {
