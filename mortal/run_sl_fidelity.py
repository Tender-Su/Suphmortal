...
import run_sl_formal as formal
...
FIDELITY_ROOT = REPO_ROOT / 'logs' / 'sl_fidelity'
...
P1_CALIBRATION_DEFAULT_MODE = sl_defaults.CURRENT_P1_CALIBRATION_MODE
P1_CALIBRATION_DEFAULT_PROTOCOL_ARMS = sl_defaults.CURRENT_P1_CALIBRATION_PROTOCOL_ARMS
P1_SINGLE_HEAD_CALIBRATION_BASELINE = dict(
    sl_defaults.CURRENT_P1_SINGLE_HEAD_CALIBRATION_BASELINE
)
P1_SINGLE_HEAD_CALIBRATION_SOURCE = sl_defaults.CURRENT_P1_SINGLE_HEAD_CALIBRATION_SOURCE
...
P1_PROTOCOL_DECIDE_COORDINATE_MODE = sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_COORDINATE_MODE
P1_PROTOCOL_DECIDE_TOTAL_BUDGET_RATIOS = list(sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_TOTAL_BUDGET_RATIOS)
P1_PROTOCOL_DECIDE_MIXES = list(sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_MIXES)
P1_WINNER_REFINE_STEP_SCALE = 1.5
P1_WINNER_REFINE_SEED_OFFSETS = [0, 1009]
P1_WINNER_REFINE_CENTER_MODE = sl_defaults.CURRENT_P1_WINNER_REFINE_CENTER_MODE
P1_WINNER_REFINE_PROTOCOL_ARM = sl_defaults.CURRENT_P1_WINNER_REFINE_PROTOCOL_ARM
P1_WINNER_REFINE_CENTER_KEEP = getattr(sl_defaults, 'CURRENT_P1_WINNER_REFINE_CENTER_KEEP', 3)
P1_WINNER_REFINE_CENTER_ARM_NAMES = tuple(
    sl_defaults.CURRENT_P1_WINNER_REFINE_CENTER_ARMS
)
P1_WINNER_REFINE_TOTAL_SCALE_FACTORS = [0.85, 1.0, 1.15]
P1_WINNER_REFINE_TRANSFER_DELTA = 0.01
P1_MAINLINE_STAGES = tuple(
    getattr(sl_defaults, 'CURRENT_P1_MAINLINE_STAGES', ('calibration', 'protocol_decide', 'winner_refine'))
)
P1_BACKLOG_STAGES = tuple(getattr(sl_defaults, 'CURRENT_P1_BACKLOG_STAGES', ('ablation',)))
P1_ABLATION_POLICY = str(getattr(sl_defaults, 'CURRENT_P1_ABLATION_POLICY', 'backlog_manual_only'))
...
P1_PROGRESSIVE_NOISE_MARGIN_MULT = (
    sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_PROGRESSIVE_NOISE_MARGIN_MULT
)
...
P1_PROTOCOL_DECIDE_PROGRESSIVE_AMBIGUITY_MODE = (
    sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_PROGRESSIVE_AMBIGUITY_MODE
)
P1_PROTOCOL_DECIDE_PROGRESSIVE_GAP_THRESHOLD = (
    sl_defaults.CURRENT_P1_PROTOCOL_DECIDE_PROGRESSIVE_GAP_THRESHOLD
)
...
P1_BUDGET_RATIO_DIGITS = int(getattr(sl_defaults, 'CURRENT_P1_BUDGET_RATIO_DIGITS', 4))
P1_AUX_WEIGHT_MIN = 0.00
P1_AUX_WEIGHT_MAX = 0.18
P1_AUX_WEIGHT_DIGITS = int(getattr(sl_defaults, 'CURRENT_P1_AUX_WEIGHT_DIGITS', 5))
        ...
        try:
            ...
            if lock_belongs_to_running_process(existing):
                raise RuntimeError(
                    ...
                    f'use `scripts\\stop_sl_fidelity.bat {run_name}` first'
                )
            try:
                lock_path.unlink()
            except FileNotFoundError:
    ...
    if P1_WINNER_REFINE_CENTER_MODE == 'explicit_arm_names':
        if protocol_arm != P1_WINNER_REFINE_PROTOCOL_ARM:
            raise RuntimeError(
                ...
                'update sl_current_defaults.py and docs before continuing'
            )
        return {
            'keep': None,
...
def update_results_doc(run_dir: Path, state: dict[str, Any]) -> None:
    ...
    if formal_state:
        ...
        if formal_state.get('ab_name'):
            lines.append(f"- 日志目录：`logs/sl_ab/{formal_state['ab_name']}`")
...
def main() -> None:
    ...
    parser.add_argument('--run-name', default='sl_fidelity_main')
