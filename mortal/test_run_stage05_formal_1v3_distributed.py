...
import run_stage05_formal_1v3_distributed as formal_1v3
...
class Formal1v3DistributedTests(unittest.TestCase):
    ...
    def test_load_formal_context_uses_formal_checkpoint_pack(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            state = {
                'formal': {
                    ...
                    'result': {
                        ...
                        'pending_canonical_alias_targets': ['C:/virtual/canonical/stage0_5_supervised.pth'],
                        'candidates': {
            ...
            self.assertEqual(
                ['C:/virtual/canonical/stage0_5_supervised.pth'],
            ...
            self.assertEqual('C:/virtual/best_acc.pth', context['candidate_index']['best_acc'].meta['checkpoint_path'])
    ...
    def test_finalize_dispatch_publishes_formal_winner(self):
        context = {
            ...
            'pending_canonical_alias_targets': ['X:/virtual/custom_cfg/checkpoints/stage0_5_supervised.pth'],
            'candidates': [],
            'candidate_order': {},
            'config_snapshot': {
                'base_1v3_cfg': {'challenger': {}, 'champion': {}},
        ...
        with (
            ...
            patch.object(
                ...
                return_value=[
                    {
                        'source': 'src',
                        'destination': 'X:/virtual/custom_cfg/checkpoints/stage0_5_supervised.pth',
                    },
                ],
            ) as publish_mock,
            patch.object(formal_1v3, 'write_dispatch_state'),
    ...
    def test_finalize_dispatch_raises_when_expected_alias_publish_is_missing(self):
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, 'did not publish any expected Stage 0\\.5 canonical aliases'):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            state = {
                ...
                'formal': {
                    'status': 'completed',
                    'result': {
                        'pending_canonical_alias_targets': [str(run_dir / 'checkpoints' / 'stage0_5_supervised.pth')],
                    },
                },
                'final_conclusion': {
    ...
    def test_sync_remote_formal_shortlist_assets_rewrites_remote_state_paths(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            run_dir = repo_root / 'logs' / 'stage05_fidelity' / 'demo_run'
            ckpt_dir = repo_root / 'logs' / 'stage05_ab' / 'formal_demo' / 'checkpoints'
        ...
        self.assertEqual(r'D:\MahjongRemote\logs\stage05_fidelity\demo_run\state.json', payload['state_path'])
        self.assertEqual(r'D:\MahjongRemote\logs\stage05_ab\formal_demo\checkpoints\best_loss.pth', payload['checkpoints']['best_loss'])
        self.assertEqual(r'D:\MahjongRemote\logs\stage05_ab\formal_demo\checkpoints\best_acc.pth', payload['checkpoints']['best_acc'])
        ...
        self.assertEqual(
            r'D:\MahjongRemote\logs\stage05_ab\formal_demo\checkpoints\best_loss.pth',
            captured_remote_state['formal']['result']['candidates']['best_loss']['path'],
        )
        self.assertEqual(
            r'D:\MahjongRemote\logs\stage05_ab\formal_demo\checkpoints\best_acc.pth',
    ...
    def test_launch_task_for_worker_uses_remote_launch_mode(self):
        ...
        with patch.object(formal_1v3, 'launch_remote_task', return_value='remote-active') as launch_remote_task:
            active = formal_1v3.launch_task_for_worker(
                ...
                dispatch_root=Path(r'C:\Users\numbe\Desktop\MahjongAI\logs\stage05_fidelity\demo_run\distributed\formal_1v3_dispatch'),
