...
def evaluate_formal_candidate(
    *,
    ...
    cfg['challenger']['name'] = str(candidate.arm_name)
    ...
    cfg['log_dir'] = str(
        ...
        / 'formal_1v3_dispatch'
        / 'eval_logs'
        / f'{round_label}__{candidate.arm_name}__{machine_label}'
    )
...
def finalize_dispatch(
    *,
    ...
    published = formal.publish_stage05_canonical_checkpoints(
        context['base_cfg'],
...
def build_remote_interactive_window_command(
    *,
    ...
    window_title = f'MahjongAI formal_1v3 {task_state["task_id"]}'
...
def launch_remote_task(
    *,
    ...
    remote_result_path = remote_dispatch_root / 'remote_results' / f'{task_state["task_id"]}.json'
    remote_runtime_root = remote_dispatch_root / 'remote_runtime' / str(task_state['task_id'])
    local_result_path = dispatch.ensure_dir(dispatch_root / 'results') / f'{task_state["task_id"]}.json'
    log_path = dispatch.ensure_dir(dispatch_root / 'logs') / f'{task_state["task_id"]}__{worker.label}.log'
