...
DEFAULT_SCRATCH_ROOT = Path(tempfile.gettempdir()) / "mahjongai_sl_ab"
...
def collect_repo_cleanup_targets(sl_ab_root: Path) -> list[Path]:
    targets: list[Path] = []
    for arm_result_path in sl_ab_root.rglob("arm_result.json"):
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Clean historical sl_ab intermediate artifacts without touching final-phase outputs.",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    sl_ab_root = args.repo_root / "logs" / "sl_ab"
    targets = collect_repo_cleanup_targets(sl_ab_root) if sl_ab_root.exists() else []
