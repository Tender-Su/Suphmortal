...
DEFAULT_RUN_NAME = 's05_fidelity_main'
...
def command_targets_run(command_line: str | None, run_name: str) -> bool:
    if not command_line or 'run_stage05_fidelity.py' not in command_line:
        return False
...
def query_stage05_processes() -> list[dict[str, object]]:
    command = (
        "$procs = Get-CimInstance Win32_Process | Where-Object { "
        + "$_.Name -match '^python(\\.exe)?$' -and "
        + "$_.CommandLine -match 'run_stage05_fidelity\\.py' "
        + "}; "
...
def find_matching_pids(run_name: str) -> list[int]:
    return [
        int(proc['ProcessId'])
        for proc in query_stage05_processes()
