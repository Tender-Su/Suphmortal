...
impl PlayerState {
    #[must_use]
    pub fn can_ron_on_tile(&self, tile: Tile) -> bool {
        self.ron_point_on_tile(tile).is_some()
    }

    #[must_use]
    pub fn ron_point_on_tile(&self, tile: Tile) -> Option<Point> {
        if self.at_furiten || !self.waits[tile.deaka().as_usize()] {
            return None;
        }

        self.agari_points_for_tile(true, tile, &[]).ok()
    }
    ...
    pub fn agari_points(&self, is_ron: bool, ura_indicators: &[Tile]) -> Result<Point> {
        ...
        self.agari_points_for_tile(is_ron, winning_tile, ura_indicators)
    }

    fn agari_points_for_tile(
        &self,
        is_ron: bool,
        winning_tile: Tile,
        ura_indicators: &[Tile],
